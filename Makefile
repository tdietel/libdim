...
install:	$(DCSDIR)/lib/i386/libdim.so \
		$(DCSDIR)/include/dim/dim.hxx \
		$(DCSDIR)/include/dim/dim_core.hxx \
		$(DCSDIR)/include/dim/dic.hxx \
		$(DCSDIR)/include/dim/dis.h \
		$(DCSDIR)/include/dim/dic.h \
		$(DCSDIR)/include/dim/dim_common.h \
		$(DCSDIR)/include/dim/sllist.hxx \
		$(DCSDIR)/include/dim/dllist.hxx \
		$(DCSDIR)/include/dim/tokenstring.hxx \

$(DCSDIR)/lib/i386/%.so: $(ODIR)/%.so
	cp $< $@

$(DCSDIR)/include/dim/%: dim/%
	mkdir -p $(dir $@)
	cp $< $@
...
clean:
	...
	/bin/rm -f $(ODIR)/libdim.a $(ODIR)/libdim.so
	/bin/rm -f $(ODIR)/dns
